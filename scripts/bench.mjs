...
const client = createOpencodeClient({
  ...
  fetch: (u, i = {}) => globalThis.fetch(u, { ...i, headers: { ...i.headers, Authorization: auth } }),
});
...
(async () => {
  for await (const ev of events.stream) {
    if (ev.type === "message.part.updated" && ev.properties?.sessionID === sid && ev.properties.part?.type === "text") {
      if (ttft === null && ev.properties.part.text.trim()) {
        ttft = Date.now() - t0;
...
await client.session.prompt({
  path: { id: sid },
  body: {
    model: { providerID: "omniroute", modelID: "auto/fast" },
    parts: [{ type: "text", text: "Responda em uma palavra: ola" }],
  },
});
