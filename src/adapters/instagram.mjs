...
export class InstagramAdapter {
  constructor({ log, allowedUserIds = null }) {
    this.log = log;
    this.name = "instagram";
    ...
    this.allowed = new Set((allowedUserIds ?? String(process.env.INSTAGRAM_ALLOWED_USER_IDS || "")
      .split(",").map((s) => s.trim()).filter(Boolean)).map(String));
    this.onMessage = null;
    this.server = null;
    this.running = false;
  ...
  async start() {
    ...
    const missing = ["META_ACCESS_TOKEN", "INSTAGRAM_ACCOUNT_ID", "META_VERIFY_TOKEN", "META_APP_SECRET"].filter(
      (k) => !process.env[k]
    );
